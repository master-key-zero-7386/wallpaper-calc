      ...
      <div style={{ display: "flex", gap: 4, marginTop: 4 }}>
        {[
          ...
          return (
            <button
              key={opt.value}
              onClick={() => onChange({ ...opening, material: opt.value })}
              style={{
                flex: "1 1 0%",
                minWidth: 0,
                fontSize: 11,
                padding: "4px 4px",
                borderRadius: 5,
                border: active ? `1px solid ${opt.color}` : "1px solid #cbd5c0",
                background: active ? `${opt.color}22` : "#f7f7f7",
                color: active ? opt.color : "#8a9a80",
...
function DirectionOpenings({ label, list, onChange }) {
  return (
    <div style={{ marginTop: 12, marginBottom: 4, paddingLeft: 10, borderLeft: "4px solid #7a9a68" }}>
      ...
      >
        {label}の壁(施工しない/別施工する部分)
      </div>
      {list.length === 0 && (
        <div style={{ fontSize: 12, color: "#9aa892", marginBottom: 4 }}>特殊部分なし</div>
      )}
      {list.map((o, i) => (
        <OpeningRow
          key={o.id}
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
        ...
        <button
          onClick={() => setField({ useOtherSheet: !scope.useOtherSheet })}
          style={{
            fontSize: 12,
            padding: "6px 10px",
            borderRadius: 5,
            border: scope.useOtherSheet ? "1px solid #b8860b" : "1px solid #9ab08c",
            background: scope.useOtherSheet ? "#f5e6b8" : "#f3f8ee",
            color: scope.useOtherSheet ? "#7a5a10" : "#4c6b40",
            fontWeight: 700,
            cursor: "pointer",
            whiteSpace: "nowrap",
            marginBottom: 10,
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: scope.cfPattern ? 8 : 0 }}>
          <button
            ...
            style={{
              ...
              padding: "7px",
              borderRadius: 6,
              border: !scope.cfPattern ? "2px solid #4c6b40" : "1px solid #cbd5c0",
              background: !scope.cfPattern ? "#e6f0dd" : "#fff",
              color: "#33502e",
              fontSize: 13,
              fontWeight: !scope.cfPattern ? 700 : 400,
              cursor: "pointer",
            }}
          >
            無地(向き自由)
          ...
          <button
            ...
            style={{
              ...
              padding: "7px",
              borderRadius: 6,
              border: scope.cfPattern ? "2px solid #b8860b" : "1px solid #cbd5c0",
              background: scope.cfPattern ? "#f5e6b8" : "#fff",
              color: "#7a5a10",
              fontSize: 13,
              fontWeight: scope.cfPattern ? 700 : 400,
              cursor: "pointer",
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                ...
                style={{
                  ...
                  padding: "6px",
                  borderRadius: 6,
                  border: scope.cfDirection === "l" ? "2px solid #b8860b" : "1px solid #cbd5c0",
                  background: scope.cfDirection === "l" ? "#f5e6b8" : "#fff",
                  fontSize: 12,
                  color: "#7a5a10",
                  cursor: "pointer",
                }}
              ...
              <button
                ...
                style={{
                  ...
                  padding: "6px",
                  borderRadius: 6,
                  border: scope.cfDirection === "w" ? "2px solid #b8860b" : "1px solid #cbd5c0",
                  background: scope.cfDirection === "w" ? "#f5e6b8" : "#fff",
                  fontSize: 12,
                  color: "#7a5a10",
                  cursor: "pointer",
                }}
      ...
      <DirectionOpenings
        label="北"
        list={scope.openings.north}
        onChange={(v) => setField({ openings: { ...scope.openings, north: v } })}
      />
      <DirectionOpenings
        label="南"
        list={scope.openings.south}
        onChange={(v) => setField({ openings: { ...scope.openings, south: v } })}
      />
      ...
      <DirectionOpenings
        label="東"
        list={scope.openings.east}
        onChange={(v) => setField({ openings: { ...scope.openings, east: v } })}
      />
      <DirectionOpenings
        label="西"
        list={scope.openings.west}
        onChange={(v) => setField({ openings: { ...scope.openings, west: v } })}
      />

      <div style={{ height: 14 }} />
      <SectionTitle>追加の壁 (増築部・間仕切りなど)</SectionTitle>
      {scope.extraWalls.map((w, i) => (
        <div key={w.id} style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
          ...
          />
          <button
            onClick={() => setField({ extraWalls: scope.extraWalls.filter((_, idx) => idx !== i) })}
            style={{
              width: 32,
              height: 32,
              borderRadius: 5,
      ...
      <button
        onClick={() => setField({ extraWalls: [...scope.extraWalls, emptyWall()] })}
        style={{
          fontSize: 13,
          padding: "5px 10px",
          borderRadius: 5,
    ...
    <div style={{ background: "#fbfdf9", border: "1px dashed #9ab08c", borderRadius: 8, padding: 12, marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, gap: 8 }}>
        <input
          ...
          style={{ fontSize: 14, fontWeight: 700, border: "none", background: "transparent", color: "#33502e", flex: "1 1 auto" }}
        />
        <button
          onClick={removeSubRoom}
          style={{
            fontSize: 12,
            padding: "4px 10px",
            borderRadius: 5,
        ...
        <div style={{ padding: 14 }}>
          ...
          {room.subRooms.map((sr, i) => (
            <SubRoomCard
              key={sr.id}
              subRoom={sr}
              setField={setSubField(sr.id)}
              removeSubRoom={() => removeSubRoom(sr.id)}
              result={result.subs[i]?.result}
          ...
          {room.subRooms.length > 0 && (
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span>床合計</span>
                <span>{round1(result.floorAreaM2)}㎡ / {round1(room.useOtherSheet ? result.otherSheetLenCm : result.cfLenCm)}cm</span>
              </div>
            </div>
          )}
...
export default function WallpaperCalcApp() {
  ...
  const defaultPanelShops = (prefix) => [
  ...
  const loadProject = async (key, name) => {
    ...
    try {
      ...
      const payload = project.payload;
      // 旧データのisAluminumPanel(真偽値)をmaterialフィールドに変換(後方互換)
      const migrateOpening = (o) => ({
        ...o,
        id: nextIdSafe(),
      ...
      const reIdOpenings = (openings) => ({
        ...
        south: openings.south.map(migrateOpening),
        east: openings.east.map(migrateOpening),
        west: openings.west.map(migrateOpening),
      });
      const reIdSubRoom = (sr) => ({
        ...sr,
        id: nextIdSafe(),
  ...
  const perRoomAluminumSheets = results.reduce(
    (s, r) => s + panelSheetsFromArea(r.aluminumPanelAreaM2, aluminumPanelWidth, aluminumPanelHeight),
    0
  );
  ...
  // 石膏ボード:壁紙数量には影響せず(壁紙はそのまま貼るため)、板材として別途枚数集計
  const totalGypsumAreaRaw = results.reduce((s, r) => s + r.gypsumBoardAreaM2, 0);
  const perRoomGypsumSheets = results.reduce(
    (s, r) => s + panelSheetsFromArea(r.gypsumBoardAreaM2, gypsumBoardWidth, gypsumBoardHeight),
    0
  );
  ...
  // アルミ複合板・石膏ボードは板材で円/枚の単価なので、m単位の仕入れ単位選択(computeShopPurchase)は使わない
  ...
  const gypsumShopSubtotals = gypsumShops.map((s) => finalGypsumSheets * num(s.price));
  const gypsumShopTotals = gypsumShops.map((s, i) => gypsumShopSubtotals[i] + num(s.shipping));
  ...
  const adoptedGypsumShopIdx = gypsumShops.findIndex((s) => s.id === adoptedGypsumShopId);
  const adoptedGypsumShop = adoptedGypsumShopIdx >= 0 ? gypsumShops[adoptedGypsumShopIdx] : null;
  const adoptedGypsumSubtotal = adoptedGypsumShopIdx >= 0 ? gypsumShopSubtotals[adoptedGypsumShopIdx] : 0;
  ...
  const exportExcel = () => {
    ...
    const rows = [
      ...
      ...(hasGypsumBoards
        ? [
            ["石膏ボード 幅(mm)", num(gypsumBoardWidth)],
      ...
      ...(hasOtherSheetRooms
        ...
        : []),
      ...(hasAluminumPanels
        ? panelShopExcelRows("アルミ複合板", aluminumShops, aluminumShopSubtotals, aluminumShopTotals, adoptedAluminumShopId)
        : []),
      ...(hasGypsumBoards
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hasAluminumPanels && (
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, opacity: 0.7, marginBottom: hasGypsumBoards ? 4 : 10 }}>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.2)", marginTop: 12, paddingTop: 12 }}>
          ...
          {hasAluminumPanels && (
            <>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", fontSize: 11, opacity: 0.65, marginBottom: hasGypsumBoards ? 6 : 0 }}>
