...
export const emptyOpening = () => ({ id: nextId(), h: "", w: "", isAluminumPanel: false });
...
export function computeRoom(room, wallpaperWidthMm, cfWidthMm, otherSheetWidthMm) {
  ...
  const aluminumPanelAreaM2 = allOpenings.reduce(
    (s, o) => s + (o.isAluminumPanel ? mm2ToM2(o.h, o.w) : 0),
    0
  );
...
// アルミ複合板は硬い板材で継続的な巻き物ではないため、ロス率は考慮せず
// 必要面積をパネル1枚の面積で割って切り上げるだけで枚数を出す。
export function aluminumPanelSheets(areaM2, panelWidthMm, panelHeightMm) {
